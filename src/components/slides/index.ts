import AboutSlide from './About/AboutSlide.vue';
import BlogSlide from './Blog/BlogSlide.vue';
import ContactSlide from './ContactSlide.vue';
import ProjectsSlide from './Project/ProjectsSlide.vue';
import TechSlide from './Tech/TechSlide.vue';
import WelcomeSlide from './WelcomeSlide.vue';

export { AboutSlide, BlogSlide, ContactSlide, ProjectsSlide, TechSlide, WelcomeSlide };
